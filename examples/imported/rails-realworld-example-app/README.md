...
This target shows that Topogram can import, reconcile, and adopt a Rails RealWorld brownfield app into a closed imported proof state.
...
export TARGET_ROOT="${TARGET_ROOT:-$(mktemp -d /tmp/topogram-demo-rails-realworld-XXXXXX)}"

rsync -a \
  "$TOPOGRAM_DEMO_REPO/examples/imported/rails-realworld-example-app/source/" \
  "$TARGET_ROOT/"
...
node ./ops/capture-verification-receipt.mjs rails-realworld-example-app --topogram-repo "$TOPOGRAM_REPO" --rerun-root "$TARGET_ROOT"
# sync examples/imported/rails-realworld-example-app/source and topogram from "$TARGET_ROOT" before refreshing metadata
node ./ops/refresh-proof-status-metadata.mjs --topogram-repo "$TOPOGRAM_REPO" rails-realworld-example-app
```
